...
function componentDidUpdateMod(file,api){
  const j = api.jscodeshift;
  const root = j(file.source);
  const componentDidUpdate = root.find(j.ClassDeclaration).find(j.MethodDefinition,{
      	key :{
        	type : "Identifier",
            name : "componentDidUpdate",
        }
  });
  if(componentDidUpdate.__paths.length){
  	let start = componentDidUpdate.__paths[0].value.value.body.start; 
 	let end = componentDidUpdate.__paths[0].value.value.body.end;

    componentDidUpdateFunc += "useEffect (() => "+ file.source.slice(start, end) + ");\n"
  }
}
...
function stateDeclarationMod(file,api){
	const j = api.jscodeshift;
  	const root = j(file.source);
  
  	let p = j(file.source).find(j.ClassDeclaration);
    var class_slice = (file.source.slice(p.__paths[0].value.start , p.__paths[0].value.end));
    ...
    const state_declaration =j(class_slice).find(j.ClassProperty,{
  	...
  	if(state_declaration.__paths.length > 0){
      var state_start =state_declaration.__paths[0].value.value.start;
      var state_end =state_declaration.__paths[0].value.value.end;                           

      setStateDecl = "const [object, objectSetter] = useState(";

      setStateDecl +=class_slice.slice(state_start, state_end);
      setStateDecl += ");";
    }	
}

function removeStateDecl(file,api){
	const j = api.jscodeshift;
  	const root = j(file.source);
    ...
    //state declarations
    return j(class_slice).find(j.ClassProperty,{
      	key :{
        	type : "Identifier",
            name : "state",
        },
...
export default function transformer(file, api) { 
    ...
    file.source = file.source.replaceAll("this.","")
  	...
  	classDecl(file,api)
  	stateDeclarationMod(file, api)
  	file.source = removeStateDecl(file, api)
  	constructorMod(file,api)
  	componentDidMountMod(file,api)
  	componentWillUnmountMod(file,api)
  	componentDidUpdateMod(file,api)
  	renderMod(file,api)
  	methodsMod(file,api)
  	...
  	return importStrings+file.source+"\n"+functionDecl+"\n"+setStateDecl+constructorFunc+componentDidMountFunc+componentDidUpdateFunc+componentWillUnmountFunc+classProperties+MethodDefinitions+renderFunc+exportDefaultDecl
